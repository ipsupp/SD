...
void ATM::adaugare_bancnote_bancomate(int elem, int frecv)
{
    bancnote_in_atm.adauga_bancnota(elem,frecv);
}

void ATM::schimbare_or_stergere_frecv(int elem, int frecv)
{
...
{
    int copie_suma = suma, nr_elemente = 0, v_banc[10] = {0}, v_freq[10] = {0};
                ...
                for (int j = 0; j < size; j++)
                {
                    if (v_banc[i] == bancnote_in_atm.get_bancnota(j))
                    {
                        ...
                        if (frecv > v_freq[i])
                            ...
                            bancnote_in_atm.sterge_bancnota(v_banc[i]);
                            j = size + 1;
                        }
                    }
                }
...
ATM& ATM::adaugare_din_fisier()
{
    cout<<"Incarcare date din fisier..."<<endl;
    ifstream fin ("/Users/Windows 11 Pro/Desktop/Laboratoare/Structuri de date/lab_04_05/date.in");
    int banc, frecv;
    while (fin>>banc>>frecv)
        bancnote_in_atm.adauga_bancnota(banc,frecv);
    cout<<"Date incarcate!"<<endl;
}