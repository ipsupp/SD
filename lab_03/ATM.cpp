...
ATM::ATM(int capacity): bancnote_in_atm(Collection(capacity))
{
    nr_tranz = 0;
//    for (int i = 0; i < 101; i ++)
//        tranzaction[i] = Tranzaction();
}
...
void ATM::tranzactie(int suma)
{
    ...
    int rezultat = copy.tranzactie(nr_elemente,copie_suma,v_banc,v_freq);
    if (rezultat == -1)
